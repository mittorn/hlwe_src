...
CPSBlastCone::CPSBlastCone(int maxParticles, float partvelocity, const Vector &origin, const Vector &direction, const Vector &spread, float size, float sizedelta, byte r, byte g, byte b, float a, float adelta, int sprindex, bool animate, int frame, int r_mode, float PartEmitterLife)
{
	index = 0;
	removenow = false;
	ResetParameters();
	...
	if ((a/-adelta) <= 0.0f)
			gEngfuncs.Con_DPrintf(" WARNING: Part Life time is : %d!\n", (a/-adelta));
	else
		m_fLife = (a/-adelta)+1.0f;
...
void CPSBlastCone::InitializeParticle(const int &index)
{
