...
//int alt_index, alt_anim, alt_frame;//LLAPb: put all lefthand shit right in here
...
void DLLEXPORT HUD_TxferLocalOverrides( struct entity_state_s *state, const struct clientdata_s *client )
{
	...
	/*
	if(gEngfuncs.GetLocalPlayer()->curstate.number == state->number)
	{
		alt_index = client->ammo_shells;//LLAPb: index
		alt_anim = client->ammo_nails;//anim
		alt_frame = client->ammo_rockets;//frame
	}
	*/
}
...
void DLLEXPORT HUD_TxferPredictionData ( struct entity_state_s *ps, const struct entity_state_s *pps, struct clientdata_s *pcd, const struct clientdata_s *ppcd, struct weapon_data_s *wd, const struct weapon_data_s *pwd )
{
	...
	gHUD.m_iPlayerFrozen = (ppcd->flags & FL_FROZEN) ?1:0;
...
void DLLEXPORT HUD_StudioEvent( const struct mstudioevent_s *event, const struct cl_entity_s *entity )
{
	switch( event->event )
	{
		case 5001: gEngfuncs.pEfxAPI->R_MuzzleFlash( (float *)&entity->attachment[0], atoi( event->options) );break;
		case 5002: gEngfuncs.pEfxAPI->R_SparkEffect( (float *)&entity->attachment[0], atoi( event->options), -100, 100 );break;
		case 5003: gEngfuncs.pEfxAPI->R_SparkEffect( (float *)&entity->attachment[1], atoi( event->options), -100, 100 );break;
		case 5004: gEngfuncs.pfnPlaySoundByNameAtLocation( (char*)event->options, 1.0, (float *)&entity->attachment[0] );break;
		case 5005: EV_EjectBrass( (float*)&entity->attachment[0], entity->angles[1], TE_BOUNCE_SHELL, atoi(event->options) ); break;
		case 5006: EV_EjectBrass( (float*)&entity->attachment[1], entity->angles[1], TE_BOUNCE_SHELL, atoi(event->options) ); break;
		case 5007: EV_EjectBrass( (float*)&entity->attachment[2], entity->angles[1], TE_BOUNCE_SHELL, atoi(event->options) ); break;
		case 5008: EV_EjectBrass( (float*)&entity->attachment[3], entity->angles[1], TE_BOUNCE_SHELL, atoi(event->options) ); break;
		...
		case 5011: gEngfuncs.pEfxAPI->R_MuzzleFlash( (float *)&entity->attachment[1], atoi( event->options) );break;
		...
		case 5013: //muzzle flashes (attach 1)
		{//Params: 1.Sprite (0-4), 2.Frame, 3.Scale
			int Spr, Frame;
			float Scale;
			sscanf(event->options, "%i %i %f", &Spr, &Frame, &Scale );
			...
			g_pRenderManager->AddSystem(new CRSSprite((float*)&entity->attachment[1], Vector(0,0,0), Spr, Frame, kRenderTransAdd, 255,255,255, 0.8, -1.0, Scale, -(Scale*2), 0.0, 0.05), 0, -1);
			break;
		}
		case 5014:
		{
		...
		case 5015: gEngfuncs.pEfxAPI->R_Implosion( (float *)&entity->attachment[0], 50, 10, 0.2 );break;

		case 5021: gEngfuncs.pEfxAPI->R_MuzzleFlash( (float *)&entity->attachment[2], atoi( event->options) );break;
		case 5031: gEngfuncs.pEfxAPI->R_MuzzleFlash( (float *)&entity->attachment[3], atoi( event->options) );break;
	default:break;
	}
}
	...
	void	( *Callback_TempEntPlaySound )( TEMPENTITY *pTemp, float damp ) )
{
	...
	while ( pTemp )
	{
		...
		else
		{
			...
			{        //no smoke in water
				if ( gEngfuncs.PM_PointContents(pTemp->entity.origin, NULL ) != CONTENTS_WATER)
				{
					...
					else if ( pTemp->entity.baseline.sequence == 666 ) // blood
