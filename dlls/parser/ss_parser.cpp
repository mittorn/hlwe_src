...
char* SkipBraketExpression ( char *begin, char *end, char open, char close )
{
	...
	while (level)
	{
		...
		else if (*iterator == close && !(close == '>' && *(iterator - 1) == '-'))//New bad shit for object-oriented operator
		{
			--level;
		}
	}
...
//We using just end delimiter, start delimiter will be ignored anyway...
Operator* ParseArray (char *begin, char *end, Scope *s, Constants *con, Functions *f, StructData *str, char end_delimiter)
{
	if (errors) return 0;
	...
	//It is allowed for constant array to be empty
	char *test = SkipSpaces (iterator, end);

	if (*test == end_delimiter && SkipSpaces (test + 1, end) == end)
	{
		return init;
	}

	if (SkipSpaces (iterator, end) == end)//It is allowed for constant array to be empty
	{
		return init;
	}
	...
	while (true)
	{
		...
		else if (*iterator == ',' || (*iterator == end_delimiter && !(end_delimiter=='>' && *(iterator-1)=='-')))
		{
...
Operator* ParseExpression ( char *begin, char *end, Scope *s, Constants *con, Functions *f, StructData *str )
{
	...
	while (true)
	{
		...
		{
			//Special shit for new object-oriented operator:
			if (*(iterator + 1) == '>')
			{
				//Create a simple call instead of new operator...
				Call *invokation = new Call (f, MESSAGING_ROUTINE);

				char *fname, *args_begin, *args_end;

				//Parse message name:
				fname = SkipSpaces (iterator + 2, end);

				if (fname == end)
				{
					ErrorMessage ("You forgot the message name?");
				...
				while (ISALNUM (*args_begin))
				{
					token [tok_pos++] = *(args_begin++);
				}
				...
				if (!ISSPACE (*args_begin) && *args_begin != '(')
				{
					char buf[256];
					sprintf (buf, "What did you done with token \"%s\"???", token);
					ErrorMessage (buf);
				...
				//Find where arguments list is beginning:
				args_begin = SkipSpaces (args_begin, end);

				if (args_begin == end)
				{
					ErrorMessage ("Did you forgot a closing brace?");
				...
				args_end = SkipBraketExpression (args_begin, end, '(', ')');

				if (!args_end || args_end == end)
				{
					ErrorMessage ("I don't know what you did, but it is completely shit!");

					delete invokation;
					return 0;
				}

				//Finally, cheat the compiler -
				//Create function with arguments instead of object-oriented operator:
				Operator *id = ParseExpression (begin, iterator, s, con, f, str);
				Constant *sel= new Constant (Type (token, STRING));
				Operator *args=ParseArray (args_begin, args_end, s, con, f, str, ')');
				...
				invokation->AddArg (id);//entindex is math expression
				invokation->AddArg (sel);//name of method is a string literal
				invokation->AddArg (args);//Arguments are just array...

				return invokation;
			}
			else//In usual case parse the subtraction operator:
			{
	...
	if (token_finished)
	{
		...
		else if (token_isnumeric)
		{
			...
			return new Constant ( Type(&iv, _INT) );
		}
...
Operator* ParseOperator ( char *begin, char *end, Scope *s, Constants *con, Functions *f, StructData *str, int permitted )
{
	...
	{
		ErrorMessage ("Declaration of constants is denied inside of functions");
...
void ParseFile ( char *fname, Scope *g, Scope *s, Constants *con, Functions *f, StructData *str )
{
	...
	fifo = fopen(address, "r");

	if (!fifo)
	{
		char msg[255];
		sprintf(msg, "file \"%s\" not found", address);
	...
	while (!feof(fifo))
	{
		if (comment_type == 0)
		{
			main_expr[pos] = fgetc(fifo);

			if (pos > 0)
			{
				if (main_expr[pos] == '*' && main_expr[pos - 1] == '/')//c-style comment
				{
					comment_type = 2;
					main_expr[pos - 1] = ' ';//comment is equal to whitespace
					--pos;
					buffer[0] = buffer[1] = '\0';
				}
				else if (main_expr[pos] == '/' && main_expr[pos - 1] == '/')//c++ comment
				{
					comment_type = 1;
					main_expr[pos - 1] = ' ';
					--pos;
	...
	ParseText (main_expr, main_expr + pos - 1, g, s, con, f, str);
