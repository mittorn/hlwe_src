...
//=============================== EXCEPTION-THROWING OPERATORS: =================================
//============================== such as break, return or continue ==============================

struct return_value//This type of exception will be caught and it's value will be returned by function
{
	Type value;

	return_value ( Type v ) : value (v) {};
};
...
public:
	...
	Type execute()
	{
		throw new return_value( value->execute() );
	};
};

struct break_level//Level of this exception will be decreased by cycle operator, that caught it. In case if level will be equal to zero, cycle, that caught this exception, will be stopped
{
	int level;

	break_level ( int l ) : level (l) {};
};
...
public:
	...
	Type execute()
	{
		throw new continue_level( level );
	};
};
